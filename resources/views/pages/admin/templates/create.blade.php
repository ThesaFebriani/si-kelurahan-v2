...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <form action="{{ route('admin.templates.store') }}" method="POST" class="bg-white shadow rounded-lg border border-gray-200 overflow-hidden">
        @csrf
        ...
        <div class="p-6 border-b border-gray-200 bg-gray-50 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="jenis_surat_id" id="jenis_surat_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    <option value="">-- Pilih Jenis Surat --</option>
                    @foreach($jenisSurat as $jenis)
                        <option value="{{ $jenis->id }}">{{ $jenis->name }} ({{ $jenis->code }})</option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center pt-8">
                ...
                <label for="is_active" class="ml-2 block text-sm text-gray-900">
                    Aktifkan Template Ini (Gunakan sebagai default baru)
                </label>
            </div>
        </div>
            ...
            <div class="flex-1 bg-gray-200 p-8 overflow-y-auto flex justify-center relative">
                ...
                <div class="w-[210mm] min-h-[297mm] bg-white shadow-xl flex flex-col">
                    ...
                    <div class="flex-1 relative">
                        <textarea name="template_content" id="template_content" class="h-full w-full border-none focus:ring-0"></textarea>
                    </div>
