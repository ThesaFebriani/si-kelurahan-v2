...
<div class="mb-8">
    ...
    <h1 class="text-2xl font-bold text-slate-800">Tambah FAQ Baru</h1>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden max-w-3xl">
    <form action="{{ route('admin.faqs.store') }}" method="POST" class="p-6 space-y-6">
        ...
        <div>
            ...
            <input type="text" name="question" id="question" value="{{ old('question') }}" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500" placeholder="Contoh: Bagaimana cara reset password?" required>
            @error('question')
        ...
        <div>
            ...
            <textarea name="answer" id="answer" rows="5" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500" placeholder="Tulis jawaban lengkap di sini..." required>{{ old('answer') }}</textarea>
            @error('answer')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="category" id="category" list="category_list" value="{{ old('category') }}" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500" placeholder="Pilih atau ketik kategori baru..." required>
            ...
            <div>
                ...
                <select name="is_published" id="is_published" class="w-full rounded-lg border-slate-300 focus:border-blue-500 focus:ring-blue-500">
                    <option value="1" {{ old('is_published') == '1' ? 'selected' : '' }}>Published (Tampil)</option>
        ...
        <div class="pt-4 flex items-center justify-end space-x-3 border-t border-slate-100">
            <button type="reset" class="px-4 py-2 border border-slate-300 rounded-lg text-slate-700 hover:bg-slate-50 font-medium transition-colors">Reset</button>
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium shadow-sm transition-all transform hover:-translate-y-0.5">Simpan FAQ</button>
        </div>
    </form>
</div>
@endsection
