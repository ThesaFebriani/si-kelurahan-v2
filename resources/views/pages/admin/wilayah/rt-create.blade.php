...
<div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border border-slate-200 p-6">
    <form action="{{ route('admin.wilayah.rt.store') }}" method="POST">
        ...
        <div class="mb-5">
            ...
            <input type="text" name="nomor_rt" id="nomor_rt" class="w-full rounded-lg border-slate-300 focus:ring-blue-500 focus:border-blue-500 @error('nomor_rt') border-red-500 @enderror" placeholder="Contoh: 001" value="{{ old('nomor_rt') }}" required>
            @error('nomor_rt')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
